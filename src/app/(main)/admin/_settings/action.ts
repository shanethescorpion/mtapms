...
export async function changeName(firstName: string, lastName: string): Promise<ActionResponseInterface>
{
  await mongodbConnect()
  try {
    const session = await getSession()
    ...
    if (!!updated?._id) {
      return {
        success: 'Admin name changed successfully',
      }
    }
  } catch (e) {
    console.log(e)
  ...
  return {
    error: 'Failed to change Admin name',
  }
}